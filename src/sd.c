...
static void spi_init(void)
{
	...
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, ENABLE);
	  ...
	  /* Enable the SPI periph */
        ...
        //RCC_APBPeriphClockCmd(RCC_APBPeriph_SPI1,ENABLE);
...
static u8 spi_txrx(u8 data)
{
	...
	//*(uint8_t *)&(SPI_SD->DR) = data;
        /*SPI_SendData8(SPI1, data);
	while ((SPI_SD->SR & SPI_SR_TXE) == 0)
        ;
	return (uint8_t)SPI_SD->DR;*/
        while(SPI_I2S_GetFlagStatus(SPI_SD, SPI_I2S_FLAG_TXE) == RESET){}
	SPI_SendData8(SPI_SD, data);
...
DWORD get_fattime()
{
	int time = 0x00; //RTC_GetTime(uint32_t BIN, RTC_TimeTypeDef* RTC_TimeStruct);
	int y, m, d;
